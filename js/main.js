...
// Build sector tabs + panels from data.js
...
function projectCard(project) {
  ...
  const imageHtml = hasImage
    ? `<img src="${project.image}" alt="${project.name}" class="project-img" onerror="this.parentElement.innerHTML = '<div class=\\'project-img placeholder\\'><span>🖼️</span><small>تعذر تحميل الصورة</small></div>';" />`
...
function sectorPanel(sector, index) {
  const cards = sector.projects.map(projectCard).join("");
  return `
